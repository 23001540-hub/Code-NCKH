...
results = []
characteristics = [] # Lưu đặc trưng để phân tích Bước 4
...
for idx, sku in enumerate(groups):
    ...
    # Tính đặc trưng chuỗi thời gian (Cho Bước 4)
    ...
    if not is_group_A:
        ...
        if len(t_qty) >= 24:
            try: evaluate('SARIMA', pm.auto_arima(t_qty, seasonal=True, m=12, suppress_warnings=True, error_action="ignore").predict(n_periods=3))
            except: pass
        ...
        # 4. Tree-based Models (XGB, LGBM, CatBoost)
        ...
        if len(df_ml) > 0:
            ...
            try:
                ...
                m_lgb = lgb.LGBMRegressor(n_estimators=30, random_state=42, verbose=-1).fit(X_tr, y_tr)
                p_lgb, hx = [], list(t_qty)
                for m in [7, 8, 9]:
                    p = m_lgb.predict(pd.DataFrame([[hx[-1], hx[-2], hx[-3], m]], columns=features))[0]
                    p_lgb.append(p); hx.append(p)
                evaluate('LightGBM', p_lgb)

                m_cat = CatBoostRegressor(iterations=30, random_state=42, verbose=0).fit(X_tr, y_tr)
                p_cat, hx = [], list(t_qty)
                for m in [7, 8, 9]:
                    p = m_cat.predict(pd.DataFrame([[hx[-1], hx[-2], hx[-3], m]], columns=features))[0]
                    p_cat.append(p); hx.append(p)
                evaluate('CatBoost', p_cat)
            except: pass
    ...
    try:
        # 1. Các mô hình Nhóm C (Truyền thống)
        if best_model == 'Naive':
        ...
        # 2. Prophet
            ...
            q1_fcst = [max(0, p) for p in full_preds[-3:]] # Lấy 3 tháng cuối của 6 tháng tương lai
        ...
        # 3. ARIMA / SARIMA
            ...
            m_f = pm.auto_arima(qty_full, seasonal=True, m=12, suppress_warnings=True, error_action="ignore")
            full_preds = m_f.predict(n_periods=6)
            q1_fcst = [max(0, p) for p in full_preds[-3:]]
        ...
        # 5. Tree-based Models (LightGBM, XGBoost, CatBoost)
        elif best_model in ['LightGBM', 'XGBoost', 'CatBoost']:
            ...
            if best_model == 'LightGBM':
                m_f = lgb.LGBMRegressor(n_estimators=30, random_state=42, verbose=-1).fit(X_full, y_full)
            elif best_model == 'XGBoost':
                m_f = xgb.XGBRegressor(n_estimators=30, random_state=42).fit(X_full, y_full)
            else:
                m_f = CatBoostRegressor(iterations=30, random_state=42, verbose=0).fit(X_full, y_full)

            # Phóng dự báo 6 tháng tương lai (T10, 11, 12, 1, 2, 3)
            hx = list(qty_full)
            ...
            for m in future_months:
                ...
                hx.append(max(0, p)) # Cập nhật lịch sử cuốn chiếu

            q1_fcst = full_preds[3:6] # Cắt lấy 3 tháng cuối (Q1/2026)

        # 6. Ensemble
        elif best_model == 'Ensemble':
            # Với ensemble, ta làm đơn giản bằng cách lấy dự báo của MA3 cho tương lai để tránh train lại 3 models nặng
            q1_fcst = [np.mean(qty_full[-3:])] * 3

    except Exception as e:
        pass # Nếu lỗi, q1_fcst mặc định giữ là [0, 0, 0] an toàn
